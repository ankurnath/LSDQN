...
class DQN:
    def __init__(self, graph_generator
        ...
        if self.gen_training_sample_first:
            self.training_instances = un_batch(self.graph_generator.generate_graph(batch_size=1, seed=11, cuda_flag=self.cuda_flag), copy=False)
        self.action_type = action_type
        ...
        if cuda_flag:
            self.model = DQNet(n=self.n, hidden_dim=self.hidden_dim).cuda()
        else:
            self.model = DQNet(n=self.n, hidden_dim=self.hidden_dim)
    ...
    def run_batch_episode(self, target_bg=None, action_type='swap', gnn_step=3, episode_len=50, batch_size=10, rollout_step=1):
        ...
        if target_bg is None:
            ...
            self.log.add_item('S_new_training_sample', torch.mean(bg.tsp_value).item())
        else:
    ...
    def sample_actions_from_q(self, prop_a_score, Q_sa, batch_size, Temperature=1.0, eps=None, top_k=1):
        ...
        if self.explore_method == 'epsilon_greedy':
            ...
            # point-mass loss
            L = -torch.log(prop_a_score[range(b), best_actions] + 1e-8) + 1.0 * (
                        torch.log(prop_a_score + 1e-8) * prop_a_score).sum(dim=1)
    ...
    def rollout(self, bg, rollout_step, top_num=5):
        ...
        for step in range(rollout_step):
            ...
            prop_a_score, prop_actions = self.model.forward_prop(bg, batch_legal_actions)
    ...
    def run_cascade_episode(self, target_bg=None, action_type='swap', gnn_step=3, rollout_step=0, verbose=False, epoch=None):
        ...
        if target_bg is None:
            ...
            self.log.add_item('S_new_training_sample', torch.mean(torch.cat([new_graphs[i].tsp_value for i in range(self.new_epi_batch_size)])).item())
        ...
        [self.cascade_replay_buffer[t].extend(
            ...
            , rewards[j+t*self.new_epi_batch_size]# + (3/4)**t
    ...
    def sample_from_cascade_buffer(self, batch_size, q_step, gnn_step, rollout_step=0, verbose=True):
        ...
        self.log.add_item('R_signal_nonzero', sum(torch.abs(R)>1e-8).item())
        self.log.add_item('R_signal_posi_len', len(R[R > 0]))
        self.log.add_item('R_signal_nega_len', len(R[R < 0]))
        self.log.add_item('R_signal_posi_mean', sum(R[R > 0]) / len(R[R > 0]))
        self.log.add_item('R_signal_nega_mean', sum(R[R < 0]) / len(R[R < 0]))
        ...
        _, R1, _, Q_s1a_ = self.model(batch_begin_state, batch_begin_action, action_type=self.action_type, gnn_step=gnn_step)
        #  action proposal network
        prop_a_score, prop_actions = self.model_target.forward_prop(batch_end_state, batch_end_action)
    ...
    def back_loss(self, R, Q, err_S, update_model=True):
        ...
        if update_model:
            ...
            self.log.add_item('Reconstruction_error', L.item())
            self.Q_err = 0
