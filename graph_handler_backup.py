...
class GraphGenerator:
    ...
    def generate_graph(self, x=None, batch_size=1, style=None, seed=None, cuda_flag=True):
        ...
        if x is None:
            if style == 'plain':
                if seed is not None:
                    ...
                    # 10 * 50 * 2 random numbers
            elif style.startswith('cluster'):
                mean = torch.rand((batch_size * k, 1, 2)).repeat(1, m, 1)
                std = (torch.rand((batch_size * k, 1, 2)).repeat(1, m, 1) + 1) * 0.1
                bg.ndata['x'] = torch.normal(mean, std).view(batch_size * n, 2)
        else:
            bg.ndata['x'] = x
        print(batch_size, n)
        # 500 tuples with position information
        ...
        if style.startswith('er') or style.startswith('ba'):
            ...
            # g = dgl.transform.knn_graph(bg.ndata['x'].view(batch_size, n, -1), ajr + 1)

            # bg.ndata['x'].view(batch_size, n, -1) 3D array 10*50*2
            # print(bg.ndata['x'].view(batch_size, n, -1))
            # print(len(bg.ndata['x'].view(batch_size, n, -1)[0]))

            # five hundred nodes, each having 50 neighbors
            # print(g)

            # for i in range(25000):
            #     print(g.edges()[0][i], g.edges()[1][i])
            # each is an edge

            # print(g.edges()[0], g.edges()[1])
            # neighbor_idx = g.nodes()
            # print(neighbor_idx)
            # square_dist_matrix = g.edges()[0]

            # neighbor_idx should have 50 elements, since each node has 50 neighbors
            # Problem 1: but whose neighbors are we looking for? x?
            # Problem 2: what is square dist matrix? the square norm distance?

            _, neighbor_idx, square_dist_matrix = new_knn_graph.knn_graph(bg.ndata['x'].view(batch_size, n, -1), ajr + 1, extend_info = True)
            # print(len(neighbor_idx[0][0])) # 50*50*1
            # print(len(square_dist_matrix[0][0])) # 50*50*1
            ...
            # bg.ndata['adj'] = torch.sqrt(square_dist_matrix.to(torch.double)).view(bg.n * bg.batch_size, -1)
