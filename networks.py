...
class GCN(nn.Module):
    ...
    def forward(self, graphs, feature, use_label=True, use_edge=False):
        ...
        # n1_h = torch.bmm(adjM.view(b, n, n).float(), feature.view(b, n, -1)).view(b * n, -1) / n  # (bn, h)
...
class DQNet(nn.Module):
    ...
    def forward_prop(self, graphs, actions=None, action_type='swap', gnn_step=3, top_ratio=0.1):
        ...
        # print(prop_action_indices_)
        prop_actions = actions[prop_action_indices_, :]
        # print(actions.shape)
        # print(prop_actions.shape)

        return prop_a_score.view(b * num_action, 1)[prop_action_indices_, :].view(b, -1), prop_actions

    def forward(self, graphs, actions=None, action_type='swap', gnn_step=3, use_attention=False, leak=False):
